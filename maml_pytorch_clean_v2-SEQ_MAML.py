...
TASK_TRIAD = ''.join([task_name[0] for task_name in args.tasks])
...
def main():
    ...
    maml = Meta(args, MODEL_CLASS, len(TOKENIZER), DEVICE)
    ...
    for task_id in range(len(args.tasks)):
        ...
        while True:
            ...
            loss_item = maml(support_x, support_y, query_x, query_y, train_loss_fct)
            ...
            if n_steps%10 == 0:
                logger.info(f'{RUN_ID} {_tasks[0]} Steps: {n_steps}/{len(train_qadata)//max_train_batch_size} Episode {n_steps}: Loss: {loss_item:.5f}  Batch: {n_inputs}')
                print(f'{RUN_ID} {_tasks[0]} Steps: {n_steps}/{len(train_qadata)//max_train_batch_size} Episode {n_steps}: Loss: {loss_item:.5f}  Batch: {n_inputs}')
